...
public async createBoardArticle(memberId: ObjectId, input: BoardArticleInput): Promise<BoardArticle> {
    // Associate the article with the member's ID
    input.memberId = memberId;

    try {
      // Create the board article
      ...
      console.error('Error in createBoardArticle:', err.message);
  ...
  public async updateBoardArticle(memberId: ObjectId, input: BoardArticleUpdate): Promise<BoardArticle> {
    ...
    const result = await this.boardArticleModel.findOneAndUpdate({_id: _id, memberId: memberId, articleStatus: BoardArticleStatus.ACTIVE}, input,  {
        new: true
    }).exec();
  ...
  public async likeTargetBoardArticle(memberId: ObjectId, likeRefId: ObjectId): Promise<BoardArticle>{
		...
		const result = await this.boardAticleStatsEditor({_id: likeRefId, targetKey: "articleLikes", modifier: modifier})
...
public async updateBoardArticleByAdmin(input: BoardArticleUpdate): Promise<BoardArticle> {
    ...
    const result = await this.boardArticleModel.findOneAndUpdate({ _id: _id, articleStatus: BoardArticleStatus.ACTIVE }, input, {
        new: true,
    })
        .exec();
   if (!result) throw new InternalServerErrorException(Message.UPDATE_FAILED);
